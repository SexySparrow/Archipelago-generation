    ...
    public static MeshData GenerateTerrainMesh(float[,] heightMap, float heightMulti, AnimationCurve meshCurve, int detail)
    {
        ...
        for (int y = 0; y < chunkSize; y += inc)
        {
            for (int x = 0; x < chunkSize; x += inc)
            {
                ...
                float height = animationCurve.Evaluate(heightMap[x, y]) * heightMulti;
                Vector3 verPos = new Vector3(topLeftX + uv.x * meshSizeNormal, height, topLeftZ - uv.y * meshSizeNormal);
